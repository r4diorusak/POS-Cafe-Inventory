 <form method="POST" action="modul/mod_kasir/kasir.php">
	<div class='box-body'>
    <table id="dataTables_minuman" class="table-bordered" style="width:100%;">
		...
		<tbody>
			<?php
			$no = 1;
			$res = mysql_query("SELECT * FROM `menu` a,kategori b where a.kategori=b.id_kategori and nama_kategori='Minuman' ORDER by a.menu DESC");
			//$res = mysql_query("SELECT * FROM `menu`");
			  while($row=mysql_fetch_array($res)){
				...
				<tr>
					...
					<td width=80px align=center><a name="btn_edit" class="btn icon-btn btn-info" href="media.php?module=p_menu&opr=upd&meja='.$_SESSION['meja'].'&penyedia='.$row['penyedia'].'&rs_id='.$row['id_menu'].'&harga='.$row['harga'].'">
